...
use windows::Win32::System::Memory::{
    VirtualAlloc, VirtualProtect, MEM_COMMIT, PAGE_EXECUTE_READWRITE, PAGE_READWRITE, PAGE_PROTECTION_FLAGS,
};

pub(crate) unsafe fn float_ptr(value: f32) -> u32 {
    let fptr = alloc_mem(4, PAGE_READWRITE) as u32;
    patch(fptr, &transmute::<f32, [u8; 4]>(value));
    return fptr
}

pub(crate) unsafe fn float64_ptr(value: f64) -> u32 {
    let fptr = alloc_mem(8, PAGE_READWRITE) as u32;
    patch(fptr, &transmute::<f64, [u8; 8]>(value));
    return fptr
}

pub(crate) unsafe fn alloc_mem(size: usize, permission: PAGE_PROTECTION_FLAGS) -> *mut c_void {
    VirtualAlloc(Some(ptr::null_mut()), size, MEM_COMMIT, permission)
}

pub(crate) unsafe fn change_permission(address: u32, size: usize, permission: PAGE_PROTECTION_FLAGS) {
...
pub(crate) unsafe fn patch(address: u32, buf: &[u8]) {
    ...
    ptr::copy(buf.as_ptr(), address as *mut u8, buf.len());
}
